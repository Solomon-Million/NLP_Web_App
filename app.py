...
#========================================================================
st.subheader("📅 Rolling Forecast to April 2025")
...
start_date = predict_df['Date'].max()
end_date = pd.Timestamp('2025-04-01')
time_window = 12

# Initialize input sequence
rolling_data = data_values.copy()
rolling_dates = list(predict_df['Date'])
predicted_prices = []
predicted_dates = []
...
while current_date < end_date:
    input_seq = rolling_data[-time_window:]
    ...
    dummy_pred = np.zeros((1, rolling_data.shape[1]))
    dummy_pred[0, 0] = prediction
    actual_prediction = scaler.inverse_transform(dummy_pred)[0, 0]

    # Append prediction
    predicted_prices.append(actual_prediction)
    current_date += pd.DateOffset(months=1)
    predicted_dates.append(current_date)

    # Update input features with predicted value
    new_row = rolling_data[-1, :].copy()
    new_row[0] = prediction  # Replace S&P500 price with prediction
    rolling_data = np.vstack([rolling_data, new_row])

# Plot rolling forecast results
fig_rolling = go.Figure()
fig_rolling.add_trace(go.Scatter(x=predict_df['Date'],
                                 y=scaler.inverse_transform(data_values)[:, 0],
                                 mode='lines', name='Historical Prices', line=dict(color="blue")))

fig_rolling.add_trace(go.Scatter(x=predicted_dates, y=predicted_prices,
                                 mode='lines+markers', name='Rolling Predictions',
                                 line=dict(color="orange", dash="dash")))

fig_rolling.update_layout(title='S&P500 Rolling Forecast to April 2025',
                          xaxis_title='Date',
                          yaxis_title='S&P500 Price',
                          hovermode='x')
...
st.success(f"### Predicted S&P500 Price for April 2025: ${predicted_prices[-1]:.2f}")
